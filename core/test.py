...
def evaluation(model, test_loader, out_loader, **options):
    ...
    # 设置阈值范围
    thresholds = np.linspace(min(min(score_close), min(score_open)), max(max(score_close), max(score_open)),                          100)
    # 初始化最大准确率
    max_accuracy = 0
    # 遍历所有阈值
    for tau in thresholds:
        tp = sum(1 for score in score_close if score >= tau)
        fp = sum(1 for score in score_open if score >= tau)
        tn = sum(1 for score in score_open if score < tau)
        fn = sum(1 for score in score_close if score < tau)
        # 计算准确率
